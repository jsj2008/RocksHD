...
{
	return [[[self alloc] initForUIView:ui bringGLViewToFront:bFront defaultViewHierStruct:bDefaultStruct] autorelease];
}
...
- (id)initForUIView:(UIView*)ui bringGLViewToFront:(BOOL)bFront defaultViewHierStruct:(BOOL)bDefaultStruct
{
    ...
    bringGLViewToFront = bFront;
    defaultViewHierStruct = bDefaultStruct;
	return self;
}
...
- (void)setParent:(CCNode *)parent
{
	if (parent == nil)
        ...
        if (defaultViewHierStruct) {
            [[[CCDirector sharedDirector] openGLView] addSubview:uiItem];
        }
        else {
            
            UIView *glView = [CCDirector sharedDirector].openGLView;
            UIView *umbrellaView = glView.superview;
        
            [umbrellaView addSubview:uiItem];
            
            if (bringGLViewToFront) {
                [umbrellaView bringSubviewToFront:glView];
